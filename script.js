...
function nextChatQuestion() {
    if (chatIndex >= chatQuestions.length) {
        ...
        setTimeout(() => {
            ...
            chatEndScreen.innerHTML = '<h2 class="glow-text" style="text-align:center;">Your answers have been sent ❤️</h2>';
...
function handleChatSend() {
    ...
    chatIndex++;
    setTimeout(nextChatQuestion, 800);
}
...
function submitToGoogleForms() {
    ...
    if (userAnswers[1]) formData.append('entry.410506505', userAnswers[1]);
    if (userAnswers[2]) formData.append('entry.1095566401', userAnswers[2]);
    if (userAnswers[3]) formData.append('entry.1902765972', userAnswers[3]);
    if (userAnswers[4]) formData.append('entry.966538598', userAnswers[4]);
    if (userAnswers[5]) formData.append('entry.699195259', userAnswers[5]);
    if (userAnswers[6]) formData.append('entry.63854159', userAnswers[6]);
    if (userAnswers[7]) formData.append('entry.292160866', userAnswers[7]);
    if (userAnswers[8]) formData.append('entry.1277309211', userAnswers[8]);
    if (userAnswers[9]) formData.append('entry.211960386', userAnswers[9]);
