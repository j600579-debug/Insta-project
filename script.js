...
function nextChatQuestion() {
    if (chatIndex >= chatQuestions.length) {
        // 1. Populate hidden form fields for FormSubmit (Email)
        const container = document.getElementById('hidden-inputs-container');
        container.innerHTML = '';
        chatQuestions.forEach((q, idx) => {
            const input = document.createElement('input');
            input.type = 'hidden';
            input.name = `Q${idx + 1}: ${q}`;
            input.value = userAnswers[idx] || 'No Answer';
            container.appendChild(input);
        });

        // 2. Submit to Google Forms (Google Sheets Backup)
        const googleFormUrl = 'https://docs.google.com/forms/d/e/1FAIpQLSfVNefmgfV4JD1g5XFfJxwuebJM8KkmbzmJB_0C6gIRNlkqAg/formResponse?t=' + Date.now();
        const formData = new URLSearchParams();
        const mappings = {
            'entry.410506505': userAnswers[0] || 'No Answer',
            'entry.1095566401': userAnswers[1] || 'No Answer',
            'entry.1902765972': userAnswers[2] || 'No Answer',
            'entry.966538598': userAnswers[3] || 'No Answer',
            'entry.699195259': userAnswers[4] || 'No Answer',
            'entry.63854159': userAnswers[5] || 'No Answer',
            'entry.292160866': userAnswers[6] || 'No Answer',
            'entry.1277309211': userAnswers[7] || 'No Answer',
            'entry.211960386': userAnswers[8] || 'No Answer'
        };
        ...
        }).catch(err => console.error("Google Forms backup failed:", err));

        // 3. UI State and FormSubmit execution

        chatInput.disabled = true;
        chatSend.disabled = true;
        chatStatus.innerText = 'Sending love to the universe...';
        ...
        // Submit the static form
        document.getElementById('love-chat-form').submit();
        return;
    }
