...
hologramCards.forEach(card => {
    card.addEventListener('click', () => {
        ...
        } else if (targetId === 'section-memories') {
            setTimeout(startMemoriesScene, 1000);
...
// =========================================
// SECTION 10: CUTE MEMORIES LOGIC
// =========================================
const memoryDialogues = [
    "Online vandhutiyaaa? ❤️",
    "Saptiya chellam 🥹",
    "Inniku romba miss pannuren 😭",
    "Nee siricha pothum ❤️",
    "Un kooda pesina night dhaan best 🌙",
    "Nee dhaan comfort 🥹❤️",
    "Call pannalamaa? ☎️",
    "Forever pesitte irukanum ❤️"
];

let memoriesInterval;

function startMemoriesScene() {
    const flowContainer = document.getElementById('dialogue-flow');
    flowContainer.innerHTML = ''; // clear any old
    ...
    gsap.to(scene.background || document.body, { backgroundColor: '#050010', duration: 2 });
    gsap.to(camera.position, { z: camera.position.z - 10, duration: 20, ease: "linear" });

    let index = 0;

    function spawnDialogue() {
        if (document.getElementById('section-memories').classList.contains('hidden-section')) {
            clearInterval(memoriesInterval);
            return;
        }

        const text = memoryDialogues[index % memoryDialogues.length];
        index++;

        // Random positions inside safe zones (avoid extreme edges)
        const topPos = 20 + Math.random() * 60; // 20% to 80%
        const leftPos = 20 + Math.random() * 60; // 20% to 80%

        const bubble = document.createElement('div');
        bubble.className = 'mem-dialogue';
        ...
        // Animate bubble in
        gsap.to(bubble, { opacity: 1, y: -20, duration: 1, ease: "power2.out" });

        // Wait 1.5s, then change to real text
        setTimeout(() => {
            bubble.innerHTML = text;
            createHeartExplosion(); // explode hearts when text appears

            // Add a little pop effect
            gsap.fromTo(bubble, { scale: 0.9 }, { scale: 1, duration: 0.5, ease: "back.out(1.5)" });

            // Fade out after 4 seconds
            setTimeout(() => {
                gsap.to(bubble, { opacity: 0, y: -40, duration: 2, ease: "power2.in", onComplete: () => {
    ...
    // Then spawn one every 4 seconds continuously
    memoriesInterval = setInterval(spawnDialogue, 4000);
}
